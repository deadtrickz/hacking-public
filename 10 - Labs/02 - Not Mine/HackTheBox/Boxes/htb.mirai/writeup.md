...
![writeup.overview.machinescli](Box%20Write-ups/htb.mirai/machinescli.png)

### Killchain
Here's the killchain (`enumeration` → `exploitation` → `privilege escalation`) for this machine:

![writeup.overview.killchain](Box%20Write-ups/htb.mirai/killchain.png)
...
![writeup.enumeration.steps.2.1](Box%20Write-ups/htb.mirai/openports.png)  

3\. From the HTTP response headers, we find that the HTTP service is running the [PiHole](https://pi-hole.net/) project:  

![writeup.enumeration.steps.3.1](Box%20Write-ups/htb.mirai/screenshot01.png)  

4\. We also find an `admin` directory from the `gobuster` scan:  

![writeup.enumeration.steps.4.1](Box%20Write-ups/htb.mirai/screenshot02.png)  

5\. Upon visiting the `http://10.10.10.48/admin/` page, we get the default PiHole dashboard:  

![writeup.enumeration.steps.5.1](Box%20Write-ups/htb.mirai/screenshot03.png)  
...
![writeup.exploitation.steps.1.1](Box%20Write-ups/htb.mirai/screenshot04.png)  

2\. We then view contents of the `user.txt` file:  

![writeup.exploitation.steps.2.1](Box%20Write-ups/htb.mirai/screenshot09.png)  
...
![writeup.privesc.steps.1.1](Box%20Write-ups/htb.mirai/screenshot05.png)  

![writeup.privesc.steps.1.2](Box%20Write-ups/htb.mirai/screenshot06.png)  
...
![writeup.privesc.steps.2.1](Box%20Write-ups/htb.mirai/screenshot07.png)  

3\. We try to extract strings from the mounted device file `/dev/sdb` and find the contents of the original `root.txt` file:  

![writeup.privesc.steps.3.1](Box%20Write-ups/htb.mirai/screenshot08.png)  
